...
func main() {
	// 向标准输出设备（屏幕）写内容
	fmt.Println("1、are u ok?——标准输出设备默认打开")

	// // 关闭标准输出设备，关闭后，fmt.Printxxx() 函数将不会在屏幕上显示内容
	// os.Stdout.Close()
	// fmt.Println("2、前面关闭了，此处不会打印")
...
func Person2() {
	// 3、读取通道中的数据，如果没有数据，阻塞不继续执行。什么时候有数据什么时候开始继续执行
	<-cusChan
	Printer("Go")
}
...
Ticker 时一个定时触发的计时器，它会以一个时间间隔（interval）不停的向内置的 channel 发送当前时间。
