...
* `delay()` 是一个挂起协程的操作，不会造成对应线程的阻塞。只能应用在协程 或 挂起函数中使用
...
上述示例代码中，虽然触发了 cancelAndJoin() , 但由于协程代码块中有计算操作，所以没有被 cancel , 而是继续执行知道程序正常结束。
...
* 显式的判断协程是否可以被取消，可以通过 isActive 判断。
...
实际应用中，通常是因为协程的执行超时了才去取消它。在 Kotlin 中我们不需要手动的去计算协程是否执行超时，可以直接通过 `withTimeout(){}` 实现超时处理。
