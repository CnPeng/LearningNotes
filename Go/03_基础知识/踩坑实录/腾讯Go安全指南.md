...
#### 2.1.1.5. 1.1.5【必须】禁止SetFinalizer和指针循环引用同时使用
...
#### 2.1.1.6. 1.1.6【必须】禁止重复释放channel
...
#### 2.1.1.9. 1.1.9【推荐】不使用slice作为函数入参
...
#### 2.1.5.1. 1.5.1【必须】敏感信息访问
- 禁止将敏感信息硬编码在程序中，既可能会将敏感信息暴露给攻击者，也会增加代码管理和维护的难度
- 使用配置中心系统统一托管密钥等敏感信息

#### 2.1.5.2. 1.5.2【必须】敏感数据输出
...
#### 2.1.7.1. 1.7.1【推荐】使用regexp进行正则表达式匹配
...
#### 3.1.2.1. 1.2.1【必须】SQL语句默认使用预编译并绑定变量
...
<a id="2.1.5"></a>
### 3.1.5. 1.5 Web跨域
#### 3.1.5.1. 1.5.1【必须】跨域资源共享CORS限制请求来源
...
#### 3.1.6.3. 1.6.3【必须】外部输入拼接到HTTP响应头中需进行过滤

- 应尽量避免外部可控参数拼接到HTTP响应头中，如业务需要则需要过滤掉`\r`、`\n`等换行符，或者拒绝携带换行符号的外部输入。

#### 3.1.6.4. 1.6.4【必须】外部输入拼接到response页面前进行编码处理
...
#### 3.1.7.1. 1.7.1【必须】安全维护session信息
...
#### 3.1.9.1. 1.9.1【必须】禁止在闭包中直接调用循环变量
